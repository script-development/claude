...
- Reads `issue_tracker_project_id` from `.claude/project-context.md` (see this plugin's own
  [README](../../README.md)) for the project to triage. If unset, discover it via `kendo-mcp`'s
  own `kendo://projects` resource and ask the user which project to triage.
...
1. **Pending reports**: `mcp__kendo__list-reports-tool` with `project_id: <project-id>, status: 0`
   (0=Pending — filtered server-side, so the response is pending-only).
2. **Project context**: `mcp__kendo__prepare-project-context-tool` with `project_id: <project-id>`
...
`PROJ-XXXX`") so you can Dismiss it with that reason. **Never reopen, update, or
...
- **Standard params**: `project_id: <project-id>`, `lane_id`: the To Do lane id from Step 1;
  ...
  `mcp__kendo__get-sprints-tool` with `project_id: <project-id>` only when the named sprint is
  not in that list.
...
- **Multi-project consumers**: default to `<project-id>` (`issue_tracker_project_id`) but respect
  any project the user names.
