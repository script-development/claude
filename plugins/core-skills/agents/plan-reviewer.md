...
Read `<plan-feature skill dir>/references/module-shape-lens.md` — `plan-feature`'s own skill
directory, given to you by the parent agent (it resolved this once for its own use in its
Phase 1.4) — focus on the "shallow-detection test" section. Then for every new Service / Action / Helper / Composable / Store / domain Model the plan introduces (skip framework-exempt artefacts: Mailables, ResourceData, FormRequest, Controller, MCP Tool, Migration, Eloquent scope, plain DTO):
