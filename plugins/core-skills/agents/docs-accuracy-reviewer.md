...
description: Audit the user-facing text a branch ships — guides, API docs, legal pages, generated LLM corpora, and skill documentation — against the code that branch actually ships. Reports per-claim verdicts (SOURCED / PARTIAL / OVER-GENERALISED / FABRICATED / SOURCE-MISSING) plus an orthogonal COMPLIANCE-CLAIM list for human review, and returns a score. Spawned by `/review-branch` Step 3 when the diff touches this project's configured `doc_paths`, and prompted for by `/pr` Step 4 on a branch with no pipeline directory.
...
**Yours** — every file in the branch diff under `<doc_paths>`, given to you below.

`<doc_paths>` is the repo's list of user-facing text prefixes, resolved by the parent agent from
this project's `.claude/project-context.md`. It holds **directory prefixes, not `**` globs** — see `/review-branch` Step 3 for why that
...
The parent agent gives you `<base>` (the diff base) and `<doc_paths>` (this project's
user-facing text prefixes, already resolved from `.claude/project-context.md` — you don't read
that file yourself).
