...
changed to prevent it from happening again. Retros live in this project's retrospectives
directory and are numbered sequentially.

This skill reads `retro_dir` from `.claude/project-context.md` — see this plugin's README
for how that file and its notation work. No file, or no field: default to `retrospectives/`.
...
```bash
ls <retro-dir>/RETRO-*.md 2>/dev/null | sort -V | tail -1
```

Increment the highest number. If no retros exist, create `<retro-dir>` and start at 001.
...
Save to `<retro-dir>/RETRO-<NNN>-<short-name>.md`.
...
```bash
git add <retro-dir>/RETRO-<NNN>-<short-name>.md
