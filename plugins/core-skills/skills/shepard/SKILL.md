...
repo's reference file beside it. A checked-in copy wins over every other source below: its team
maintains it. A repo that ships a PR driver under another name wins over this skill too; say so
and hand over.

Resolve `<skill dir>`, used throughout below, in this order — checked-in copy, then this
plugin's install, then a user-level install — and keep the first match:
