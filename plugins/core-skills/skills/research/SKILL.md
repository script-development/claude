...
outputs live in this project's research directory and can be indexed for future reference.

This skill reads `research_dir` from `.claude/project-context.md` — see this plugin's README
for how that file and its notation work. No file, or no field: default to `research/`.
...
Save to `<research-dir>/<descriptive-name>.md`.
...
```
Filed: <research-dir>/<name>.md (<word count> words)
