...
**`wireframe-reviewer`** (bundled with this plugin) consumes this file immediately after
generation — screen coverage, token validity, component references, internal consistency,
AC traceability (score ≥ 7 to proceed). **Token format must be exact** — it matches tokens
literally.

Its real audience after that is the implementer. No pre-PR agent greps this spec against the
shipped component templates, so wireframe-vs-implementation fidelity rests on whoever builds
the screen and on the specs they write. Make the file precise enough for a human to diff by eye.
...
plugin, shared with `next` and `task-writer`), then read `PLAN.md`. Extract the frontend PR
file list, acceptance criteria, and scope.
...
Once `WIREFRAMES.md` is saved, spawn the `wireframe-reviewer` agent (bundled with this plugin)
with the plan directory path. It scores 1-10 on screen coverage, token
