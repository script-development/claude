...
|---|---|---|---|
| `{{ISSUE_KEY_PREFIX}}-0461-listeners-apply-payload-directly` | (no prefix) | `{{ISSUE_KEY_PREFIX}}-0461-listeners-apply-payload-directly` | `docs/plans/{{ISSUE_KEY_PREFIX}}-0461-listeners-apply-payload-directly/` |
...
| `feature/{{ISSUE_KEY_PREFIX}}-0530-english-user-stories-9k2Bx` | `{{ISSUE_KEY_PREFIX}}-0530-english-user-stories-9k2Bx` | `{{ISSUE_KEY_PREFIX}}-0530-english-user-stories` | `docs/plans/{{ISSUE_KEY_PREFIX}}-0530-english-user-stories/` |
| `{{ISSUE_KEY_PREFIX}}-0463-foo` (directory only contains `{{ISSUE_KEY_PREFIX}}-0463-foo-bar-baz`) | `{{ISSUE_KEY_PREFIX}}-0463-foo` | `{{ISSUE_KEY_PREFIX}}-0463-foo` | `docs/plans/{{ISSUE_KEY_PREFIX}}-0463-foo-bar-baz/` (via step 5 prefix match) |
...
`/catchup` uses a simpler algorithm — issue key only (`{{ISSUE_KEY_PREFIX}}-XXXX` from the branch, no slug). This is deliberate: catchup just needs to find _any_ artefacts for the branch quickly and tolerate the imprecise match. Don't unify it with this algorithm.
