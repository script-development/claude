...
Output the eight-row table verbatim to the developer with marks and citations filled in. It stays in the conversation — see below.
...
The table is not persisted into PLAN.md. It used to be, as `## Planning Evidence`, on the argument that `plan-reviewer` reads the file and not the conversation — but `plan-reviewer` never read that section; it grades Key Design Decisions, module shape, arch-test fit and the acceptance criteria directly. The implementing session, the plan's other reader, needs the sections the table proves complete, not the proof. What the gate produces is the ✓ marks; what PLAN.md carries is the content they vouch for.

## Phase 1.6 — security & cost surface gate

**The rule:** seven rows. For each row, the planner writes a **prose answer to the row's questions** ending in a Proof line, or marks `N/A — <one-line reason>`. Empty cells are not allowed; paraphrasing the questions back is not an answer. Fail-closed.
...
- **False N/A on Row 7 for a client-touching plan.** A plan whose Approach lists a component, a store or a composable cannot mark Client-side state N/A. Search the Approach for those file shapes before accepting the N/A.
- **A Proof line with no case in it.** "Covered by the existing suite", "the spec will assert it" and a case name that does not exist are THIN. So is a case whose mock replays the answer — a transaction mock that invokes the closure once cannot prove retry safety.
...
The N/A carve-out exists because pure-content plans (translation work, refactor-only changes, backend-only features) legitimately have empty Wireframes / Site Docs Sync sections, and treating those as THIN would block them. Migration / Schema Changes and Sweep Inventory are different: with no content the heading is omitted entirely, not marked N/A. An explicit N/A with a reason is substance — silent emptiness or "TBD" is not.
